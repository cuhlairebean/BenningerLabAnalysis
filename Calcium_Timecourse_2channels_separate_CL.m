...
% it was designed for 2 channels being imaged at the same time (i.e. colocalization of IF)
% and **plots them both separately**

% second channel labeled 'delta'
...
delta_RawImg = images(:,:,2); % FOR SECOND CHANNEL (RED)
...
delta_ref = mat2gray(delta_RawImg);
...
wholeislet_labels = figure, imshow(ref_pic)
title('Whole Islet')
ci = 1;
for c = 1:numcells
...
saveas(wholeislet_labels,[output_dir '/' filename '_' 'whole islet labels' '.tif']);


%% Colocalization - SST channel 2
...
numcells = 1;
        ...
        k = 1;
        while k > 0
            disp('Draw ROIs Around Cell of Interest')
            ROIMask = imfreehand(); %User draws region around cell
            ROIMask = createMask(ROIMask); %Mask is created from drawn region
            CellMask = CellMask + ROIMask.*numcells; %CellMask array is updated with new mask; new mask is multiplied by the cell label before updating
            CellMask(find(CellMask>numcells)) = numcells; %If a region is overlapped, it is instead attributed to the most recent region
...
for i = 1:numcells
    ...
    CellTC(:,i) = TC; %Updates CellTC array with mean intensities from each frame of each cell
    PlotLabels{i} = ['Delta Cell ' num2str(i)]; %Updates labels with current cell number for legend
end
toc
...
delta_labels = figure, imshow(delta_ref)
title ('Delta Cells')
ci = 1;
for c = 1:numcells
    text(Cent(ci,1),Cent(ci,2),num2str(c),'Color','w'); %Labels cells in the image with their respective region number
...
saveas(delta_labels,[output_dir '/' filename '_' 'delta cell labels' '.tif']);

%% All Variables %%

T = table(All_TC, delta_ca);
Ca_all = [All_TC delta_ca];

save([output_dir '/mixedCaWaveForm.mat'],'Ca_all'); % same cawaveform - Cell TC
